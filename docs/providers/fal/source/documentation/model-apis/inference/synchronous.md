...
**Source:** https://fal.ai/docs/documentation/model-apis/inference/synchronous.md
...
> **📝  Note:** In JavaScript, `fal.run()` does not currently support `timeout`, `hint`, or `headers`. It does support `startTimeout`. To use `hint`, `headers`, or `priority` from JavaScript, use [`fal.queue.submit()`](/documentation/model-apis/inference/queue#submit-parameters) or [`fal.subscribe()`](#subscribe-parameters).
