...
**Source:** https://fal.ai/docs/documentation/model-apis/inference/index.md
...
> **💡  Tip:** These calling patterns work identically for your own deployed serverless apps. Use your app's endpoint ID (e.g., `your-username/your-app-name`) instead of a model ID. See [Calling Your Endpoints](/documentation/development/calling-your-endpoints) for serverless-specific examples.
...
> **⚠️  Warning:** For production use, prefer `subscribe` or `submit` so you get automatic retries and request durability through the [queue](/documentation/model-apis/inference/queue).

- **[Learn more](/documentation/model-apis/inference/synchronous)** — Direct and queue-backed synchronous calls
...
- **[Learn Async Inference](/documentation/model-apis/inference/queue)** — The recommended way to call models at scale
...
> **📝  Note:** The `stream()` method connects to the `/stream` path on the model endpoint. Not all models support streaming. Check the model's API documentation for availability.

- **[Learn Streaming](/documentation/model-apis/inference/streaming)** — Receive output as it's generated
...
- **[Learn Real-time](/documentation/model-apis/inference/real-time)** — WebSocket connections for interactive apps
...
Before calling any model, install and configure the fal client for your language. If you are building a browser-based app, you will also need a server-side proxy to keep your API key out of client-side code.
  ...
  - **[Client Setup](/documentation/model-apis/inference/client-setup)** — Install and configure the fal client

  - **[Proxy Setup](/documentation/model-apis/inference/proxy-setup)** — Keep your API key secure in client-side apps
