...
**Source:** https://fal.ai/docs/documentation/model-apis/inference/real-time.md
...
> **⚠️  Warning:** Only models that explicitly support real-time inference can be used with the realtime client. Standard queue-based models do not have a realtime endpoint.

## Supported Models

  - **[fast-lcm-diffusion](https://fal.ai/models/fal-ai/fast-lcm-diffusion)** — SDXL with Latent Consistency Models

  - **[fast-turbo-diffusion](https://fal.ai/models/fal-ai/fast-turbo-diffusion)** — Optimized SDXL Turbo
...
- **[Proxy Setup](/documentation/model-apis/inference/proxy-setup)** — Learn how to set up a server-side proxy
...
> **⚠️  Warning:** **Protect your token endpoint with authentication.** The endpoint that generates fal tokens should verify that the request comes from an authenticated user in your application. Without proper authentication, anyone could use your endpoint to generate tokens and consume your fal credits.
...
> **📝  Note:** The `tokenProvider` also works for streaming with `connectionMode: "client"`:
