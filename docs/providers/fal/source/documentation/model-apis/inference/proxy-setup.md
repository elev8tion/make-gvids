...
**Source:** https://fal.ai/docs/documentation/model-apis/inference/proxy-setup.md
...
```
  ...
  - **[Full Next.js Example](/examples/integrations/nextjs)** — Complete walkthrough with working code

  - **[Vercel Integration](/examples/integrations/vercel)** — Deploy AI apps on Vercel with fal
...
> **⚠️  Warning:** The default configuration allows all endpoints and unauthenticated requests for backwards compatibility. For production, set `allowedEndpoints` to restrict which models can be called through your proxy and `allowUnauthorizedRequests: false` to require authentication.

> **📝  Note:** The old `handler` and `route` exports from `@fal-ai/server-proxy/nextjs` are deprecated. Migrate to `createPageRouterHandler()` and `createRouteHandler()` respectively.
...
> **💡  Tip:** The proxy URL should be relative to your application root. The client automatically constructs the full URL based on the current origin.
